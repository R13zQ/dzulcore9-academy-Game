...
class AudioSynthEngine {
  ...
  playNodeSound() {
    ...
    osc.type = 'sine';
    // Fast frequency sweep for digital sound
  ...
  // Laser Gate Passed successfully sound (major arpeggio)
  ...
  // Rhythmic Synthwave loop (Phonk/Cyberpunk Bassline)
  startMusic() {
    this.init();
    ...
    const playStep = () => {
      ...
      // Dynamic tempo adjustment based on game speed
      const currentBpm = 125 * (state.speed / CONFIG.baseSpeed);
      const currentStepDuration = 60 / currentBpm / 2;
      
      const pitch = bassline[this.step % bassline.length];
      
      // Synthesize rhythmic bass node
      const osc = this.ctx.createOscillator();
      const gain = this.ctx.createGain();
      ...
      gain.gain.exponentialRampToValueAtTime(0.001, now + currentStepDuration * 0.9);
      ...
      gain.connect(this.ctx.destination);
      osc.start(now);
      osc.stop(now + currentStepDuration * 0.9);
      
      // Add hi-hats on off-beats for synthwave beat
      if (this.step % 4 === 2) {
        const hhOsc = this.ctx.createOscillator();
        const hhGain = this.ctx.createGain();
        hhOsc.type = 'triangle';
        hhOsc.frequency.setValueAtTime(10000, now);
        hhGain.gain.setValueAtTime(0.008, now);
        hhGain.gain.linearRampToValueAtTime(0.0001, now + 0.05);
      ...
      // Riser melodies during Overdrive mode
      if (state.overdriveActive && this.step % 8 === 0) {
        const leadOsc = this.ctx.createOscillator();
        const leadGain = this.ctx.createGain();
        leadOsc.type = 'square';
        leadOsc.frequency.setValueAtTime(pitch * 4, now); // 2 Octaves up
        leadOsc.frequency.exponentialRampToValueAtTime(pitch * 8, now + currentStepDuration * 2);
        leadGain.gain.setValueAtTime(0.02, now);
        leadGain.gain.exponentialRampToValueAtTime(0.001, now + currentStepDuration * 2);
        
        leadOsc.connect(leadGain);
        leadGain.connect(this.ctx.destination);
        leadOsc.start(now);
        leadOsc.stop(now + currentStepDuration * 2);
      }

      this.step++;
      
      // Schedule next step dynamically based on tempo duration
...
function initThree() {
  ...
  // Load player jet texture
  const textureLoader = new THREE.TextureLoader();
  playerPlaneTexture = textureLoader.load('gambar_2_transparent.png');
...
function createPlayer() {
  ...
  // Create textured jet plane (matching 1024:1024 aspect ratio, width=1.4, height=1.4)
  const jetGeo = new THREE.PlaneGeometry(1.4, 1.4);
  const jetMat = new THREE.MeshStandardMaterial({
    ...
    roughness: 0.4,
    metalness: 0.7,
    color: skin.color, // Tint the jet with the skin color
    emissive: skin.color,
  ...
  // Hover position (center at y=0.86, bottom at y=0.16)
  jetMesh.position.set(0, 0.86, 0);
  // Face camera tilt
  jetMesh.rotation.x = -0.10; 
  jetMesh.castShadow = true;
  playerMesh.add(jetMesh);
  
  // Thruster light (casting neon glow on ground from exhaust engine nozzles at y=0.51)
  playerThrusterLight = new THREE.PointLight(skin.trail, 2.5, 6);
  playerThrusterLight.position.set(0, 0.51, 0.1);
...
function createTrailParticles(color) {
  ...
  for (let i = 0; i < pCount; i++) {
    ...
    positions[i * 3 + 1] = 0.51; // Align with jet exhaust nozzles
...
function animatePlayerTrail() {
  ...
  for (let i = 0; i < count; i++) {
    ...
    positions[i * 3 + 1] = 0.51 + (Math.random() - 0.5) * 0.05; // Align with jet exhaust nozzles
